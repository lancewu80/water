<%@page import="java.io.*,java.net.*,java.text.SimpleDateFormat,java.util.Date,org.json.*"%>
...
String token = request.getParameter("token");
	...
	if (cookies != null) {
		for (Cookie cookie : cookies) {
			if ("PUBLIC_APP_USER_SSO_TOKEN".equals(cookie.getName())) {
...
logBuf.append("  GET loginName  : ").append(loginName).append("\n");
logBuf.append("  GET token  : ").append(token).append("\n");
logBuf.append("  cookie for PUBLIC_APP_USER_SSO_TOKEN  : ").append(cookieValue).append("\n");
...
try {
    ...
    if (loginName == null) {
        ...
        if (!rawBody.isEmpty()) {
            ...
            logBuf.append("  [parsed] token=").append(token).append("\n");
        }
    }
    ...
    if (loginName == null) {
        ...
        success = validate(loginName, session, token, logBuf);
...
public boolean validate(String loginName,
    ...
    try {
        ...
        // ----- Step 1: 取得 PRIVILEGED_APP_SSO_TOKEN-----
        ...
        if (privateAppToken == null || privateAppToken.isEmpty()) {
            log.append("[FAIL] Step1 PRIVILEGED_APP_SSO_TOKEN 欄位缺失或為空，完整回應: ").append(resAuth).append("\n");
            return false;
        }
        log.append("[SSO] Step1 取得 PRIVILEGED_APP_SSO_TOKEN 成功\n");
