...
/* ===========================================================================
   4. TsTimer — daily 02:00 schedule
      Cron expression: 0 0 2 * * ?  (second=0 minute=0 hour=2 every day)
      Runs before the 04:00 database backup window.
   =========================================================================== */
IF NOT EXISTS (SELECT 1 FROM [dbo].[TsTimer] WHERE [FCode] = 'HR.SyncHrData')
BEGIN
    INSERT INTO [dbo].[TsTimer] (
        [FId],
        [FCode],
        [FName],
        [FRoutineClassName],
        ...
        'HR.SyncHrData',
        N'人事系統同步排程',
        'com.ai3.ecp.cus.hrsync.timer.SyncHrDataRountine',
        '0 0 2 * * ?',
        1,
        '21e2754a-d586-4bfa-bebe-768f70b9148b',
        N'每日凌晨 2 點執行人事系統同步作業（於資料庫備份前完成）'
    )
END
GO
...
-- Departments
--   DEPARTMENTID starts with '00' → selected by sync (總處 headquarters)
--   '001001' = 總處,  '001002' = 資訊處 (IT dept — triggers sysAdmin role)
--   '100001' = other dept — will NOT appear in v_KM_USER query (filtered out)
...
--   E004       → 其他處               (NOT included — DEPARTMENTID starts with '10')
